...
public class EyeTracking extends AppCompatActivity {
    ...
    private void showToast(final String msg, final boolean isShort) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(EyeTracking.this, msg, isShort ? Toast.LENGTH_SHORT : Toast.LENGTH_LONG).show();
            }
        });
    }
...
/*
    private void handleGazeInteraction(GazeInfo gazeInfo) {
        float x = gazeInfo.x;
        float y = gazeInfo.y;

        long currentTime = System.currentTimeMillis();

        // 각 버튼에 대해 시선이 머물렀는지 확인
        checkGazeOnButton(btnInitGaze, x, y, currentTime, new Runnable() {
            @Override
            public void run() {
                initGaze();
            }
        ...
        checkGazeOnButton(btnReleaseGaze, x, y, currentTime, new Runnable() {
            @Override
            public void run() {
                releaseGaze();
            }
        ...
        checkGazeOnButton(btnStartTracking, x, y, currentTime, new Runnable() {
            @Override
            public void run() {
                startTracking();
            }
        ...
        checkGazeOnButton(btnStopTracking, x, y, currentTime, new Runnable() {
            @Override
            public void run() {
                stopTracking();
            }
        ...
        checkGazeOnButton(btnStartCalibration, x, y, currentTime, new Runnable() {
            @Override
            public void run() {
                startCalibration();
            }
        ...
        checkGazeOnButton(btnStopCalibration, x, y, currentTime, new Runnable() {
            @Override
            public void run() {
                stopCalibration();
            }
        ...
        checkGazeOnButton(btnSetCalibration, x, y, currentTime, new Runnable() {
            @Override
            public void run() {
                setCalibration();
            }
        ...
        checkGazeOnButton(btnGuiDemo, x, y, currentTime, new Runnable() {
            @Override
            public void run() {
                showGuiDemo();
            }
...
*/
    ...
    /* 시선이 버튼 위에 있는지 확인하는 메소드
    private void checkGazeOnButton(Button button, float gazeX, float gazeY, long currentTime, Runnable action) {
        // 버튼의 위치를 가져옵니다.
        ...
        if (gazeX >= left && gazeX <= right && gazeY >= top && gazeY <= bottom) {
            if (!gazeStartTimeMap.containsKey(button)) {
                // 시선이 처음으로 버튼 위에 올라왔을 때
                gazeStartTimeMap.put(button, currentTime);
                Log.i(TAG, button.getText() + " 시선 시작: " + currentTime);
            } else {
                long gazeStartTime = gazeStartTimeMap.get(button);
                Log.i(TAG, button.getText() + " 시선 유지 시간: " + (currentTime - gazeStartTime) + " ms");

                if (currentTime - gazeStartTime >= GAZE_HOLD_DURATION) {
                    Log.i(TAG, button.getText() + " 시선 고정 시간 충족: " + (currentTime - gazeStartTime) + " ms");
                    action.run();
                    gazeStartTimeMap.remove(button); // 동작을 수행한 후 시선 시작 시간을 초기화합니다.
                }
            }
        } else {
            if (gazeStartTimeMap.containsKey(button)) {
                long gazeStartTime = gazeStartTimeMap.get(button);
                Log.i(TAG, button.getText() + " 시선 이동: " + (currentTime - gazeStartTime) + " ms 동안 버튼 위에 있었습니다.");
                gazeStartTimeMap.remove(button); // 시선 시작 시간을 초기화합니다.
            }
        }
    }
    */
